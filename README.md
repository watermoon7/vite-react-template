...
A two-person kanban board: boards with **Not started / In progress / Completed** columns, drag-and-drop tasks, search across every board, shared and personal notes, live sync between users, local backups, and a light/dark theme.
...
Everything tunable is in [`app.config.ts`](app.config.ts): users, columns, priorities, themes, session length, login lockout, autosave delay, backup interval/retention, search limits, and the interface scale steps.
