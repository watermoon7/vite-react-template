...
- **Boards** — the left panel lists boards; either user can add, rename (the ✎ that appears on hover, or click the title on the board itself) or delete one. Deleting anything — a board, channel, task or message — asks for confirmation in a small popover beside the button rather than a browser dialog. The board header's **Sort** control orders each column by priority, due date or name (ties keep the manual order); *Manual* is the drag-and-drop order. Dragging still works while sorted — a card dropped in a column takes the position the sort gives it, gliding there from where it was dropped rather than jumping ([`slideCards.ts`](src/react-app/slideCards.ts) plays the difference between the two as an animation). The choice is saved per browser.
- **Tasks** — a task opens in a panel on the right (which slides in when it appears, and stays put when it is handed another task) holding its description, notes, priority, due date, who it is for, and its **subtasks**: a checklist ticked off inside the task, with the count shown on the card in the column. Everything autosaves. The checklist is one field of the task rather than a collection of its own — it is stored as JSON on the task's row, travels with the task in the local backups, and is replaced as a whole on every edit, so two people editing the same task's subtasks at the same moment is last-write-wins, as it already is for its notes.
